    ...
    partial class BL : BlApi.IBL
    {
        ...
        public int helpbasestation(BO.DroneToList drone, IEnumerable<BO.BaseStation> baseStationsBL)
        {
          double distance = DistanceTo(baseStationsBL.First().BaseStationLocation.Latitude, baseStationsBL.First().BaseStationLocation.Longitude, drone.location.Longitude, drone.location.Longitude);
            ...
            {
                if (Math.Abs( DistanceTo(drone.location.Latitude, drone.location.Longitude, item.BaseStationLocation.Latitude,item.BaseStationLocation.Longitude))< distance)
                {
                    distance = Math.Abs(DistanceTo(drone.location.Latitude, drone.location.Longitude, item.BaseStationLocation.Latitude, item.BaseStationLocation.Longitude));
